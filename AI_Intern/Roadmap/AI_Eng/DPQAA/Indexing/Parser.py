...
class Chunk:
    ...
    heading_type: str
    level: int
    parent_id: str | None = None # Should store the id of the parent
    root_id: str | None = None # The id of the root
    content: str = ""
    full_content: str = ""
...
class Parser:
    ...
    def chunkDocHardCoded(self, result) -> list[Chunk]:
        '''
        Hard coded method to chunk internal FPT documents
        '''
        chunks: list[Chunk] = []
        ...
        roots = [chunk for chunk in chunks if chunk.parent_id is None]

        # Build complete content for each section and subsection
        for root in roots:
            root.full_content = self.recursive_build_root_content(chunk=root, 
                                              chunks_by_id=chunksIds)
    ...
    def pre_filter_hardCoded(self, page_content: str) -> str:
        ...
        '''
        with open("Supplies/unwanted.txt", "r") as rf:
            unwanted = [line.strip() for line in rf if line.strip()]
    ...
    def recursive_build_root_content(self, chunk: Chunk, chunks_by_id: dict[str, Chunk]) -> str:
        '''
        Recursively add content of subsections to the root (level 0) owner.
        '''
        ...
        for child_id in chunk.children:
            ...
            child_content = self.recursive_build_root_content(chunk=child, chunks_by_id=chunks_by_id)
